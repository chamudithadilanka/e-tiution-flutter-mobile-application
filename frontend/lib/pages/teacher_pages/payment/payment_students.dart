...
import 'package:frontend/pages/teacher_pages/payment/single_page_student.dart';
import 'package:http/http.dart' as http;
import 'package:frontend/api/api_service.dart';
import 'package:frontend/utils/colors.dart';
import 'package:loading_animation_widget/loading_animation_widget.dart';
...
class StudentListPayment extends StatefulWidget {
  final String classId;

  const StudentListPayment({super.key, required this.classId});

  @override
  State<StudentListPayment> createState() => _StudentListPaymentState();
}

class _StudentListPaymentState extends State<StudentListPayment> {
  final ApiService apiService = ApiService();
  final Map<String, String> attendanceStatus = {}; // studentId -> status
  List<String> statusOptions = ['Select', 'present', 'absent', 'late'];
  late Future<List<dynamic>> _studentsFuture;
                    ...
                    child: GestureDetector(
                      onTap: () {
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder:
                                (context) => SinglePageStudent(
                                  classId: widget.classId,
                                  studentId: studentId,
                                  studentFullName: fullName,
                                  studentProfileImage: profileImageUrl,
                                  studentEmail: email,
                                ),
                          ),
                        );
